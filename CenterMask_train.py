...
from Netlib.CenterMasklib.centermask.config import centermask_config, color, setup_seed
...
from Netlib.CenterMasklib.centermask.train_net.tools.dataset import (
    CentermaskCONICData,
    CentermaskPNCData,
    CenterMaksMoNuSegDS,
    DataLoader,
...
from Netlib.CenterMasklib.centermask.train_net.tools.metric import instance2results
from Netlib.CenterMasklib.centermask.train_net.tools.visualize_utils import Visualizer
...
class Moudule_process:
    def __init__(self, moudule: torch.nn.Module, dataset) -> None:
        ...
        if dataset =="monuseg":
            print("monuseg")
            data_set = CenterMaksMoNuSegDS(arg.DATASET_DIR,[
            ...
            self.train_set = Subset(data_set, train_indexes)
            self.val_set = Subset(data_set, val_indexes)
            print(len(train_indexes),len(self.train_set))
...
def _train(
    ...
    print(f"losses:{losses:.4f},lr:{lr_s.get_last_lr()[0]:.4f},epoch:{lr_s.last_epoch}")
    lr_s.step()
    return losses


@torch.no_grad()
def _val(datas: DataLoader, model: torch.nn.Module):
    # 显示模型在验证集的表现
    model.eval()
    bar = tqdm(datas, colour="red", disable=True)
    losses = []
    s2_boxes_iou = []
    s2_f1 = []
    s2_cls_acc = []
...
class Traning_process:
    ...
    def run(self):
        ...
        for _ in range(arg.epoch):
            ...
            torch.cuda.empty_cache()
            writer.add_scalar("CenterMask Training loss", losses, _)
            if _ % 30 == 0:
