...
import json
import torch
from Netlib.MaskRCNNlib.Config import mrcnn_configs,setup_seed
...
from Netlib.MaskRCNNlib.tools.dataset import (
    DataLoader,
    Subset,
    MonusegDataset,
    CPMDataset,
...
from Netlib.MaskRCNNlib.tools.visualize_utils import Visualizer
...
parser.add_argument("--config", type=str, default="MrcnnMonuSeg")
process_arg = parser.parse_args()
arg = getattr(mrcnn_configs, process_arg.config)(
    ds=process_arg.ds,
...
class Moudule_mode:
    def __init__(self, dataset) -> None:
        ...
        if dataset == "monuseg":
            self.train_set = MonusegDataset(masks_size=arg.ROI_RESOLUTION,transfs=[
                    MyRandomCrop((256,256)),
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.3)),
                    MyColorjitter(0.1, 0.1, 0.1, 0.1),
            ])
            self.val_set = Subset(self.train_set, arg.VAL_SET_INDEX)
            self.train_set = Subset(self.train_set, arg.TRAIN_SET_INDEX)
        if dataset == "cpm":
            self.train_set = CPMDataset(arg.DATASET_FP, masks_size=arg.ROI_RESOLUTION, transfs=[
                    MyRandomCrop((256,256)),
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.3)),
                    MyColorjitter(0.1, 0.1, 0.1, 0.1),
            ...
            self.train_set = Subset(self.train_set, arg.TRAIN_SET_INDEX)
        if dataset == "dsb":
            self.train_set = DSBDataset(arg.DATASET_FP, masks_size=arg.ROI_RESOLUTION, transfs=[
                    MyRandomCrop((256,256)),
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.3)),
                    MyColorjitter(0.1, 0.1, 0.1, 0.1),
