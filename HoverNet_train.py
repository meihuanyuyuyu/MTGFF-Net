...
from Netlib.HoVerNetlib.model.HoverNet import val_np_tp_hv
from Netlib.HoVerNetlib.tools.metric import jaccard, mask_category_acc
from Netlib.CDNetlib.tools.augmentation import Random_flip, MyGausssianBlur, MyColorjitter,MyCenterCrop,MyRandomCrop
from torch.cuda.amp import GradScaler, autocast
from Netlib.HoVerNetlib.tools.utils import dice_loss, focal_loss
from Netlib.HoVerNetlib.tools.dataset import (
    HoverNetCoNIC,
    HoverNetDataset_CPM,
    HoverNetDataset_DSB,
    HoverNetDataset_MoNuSeg,
    PannukeHover,
    DataLoader,
    Subset,
)
...
parser.add_argument("--config", type=str, default="HoverNetMoNuSeg")
...
def _val(data: DataLoader, model: torch.nn.Module):
    ...
    for i, data in enumerate(bar):
        ...
        with autocast():
            ...
            loss_tp = focal_loss(pred_tp, tp) + dice_loss(pred_tp, tp)
            loss_np = 2 * (focal_loss(pred_np, np) + dice_loss(pred_np, np))
            loss_hv = 3 * mse_loss(pred_hv, hv)
            loss: torch.Tensor = loss_tp + loss_np + loss_hv
            ...
            ious.append(iou)
            acces.append(acc)
            bar.set_description(f"masks iou:{iou:.4f},category acc:{acc:.4f},loss:{loss.item():.4f}")
    return torch.tensor(ious).mean().item(), torch.tensor(acces).mean().item(),torch.tensor(losses).mean().item()
...
class Module_process:
    def __init__(self, module: torch.nn.Module, dataset) -> None:
        ...
        if dataset == 'monuseg':
            data_set = HoverNetDataset_MoNuSeg(arg.DATASET_FP, arg.img_size, tranfs=[
        ...
        if dataset == 'dsb':
            data_set = HoverNetDataset_DSB(arg.DATASET_FP, arg.img_size, tranfs=[
                    MyRandomCrop((256,256)),
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.6)),
                    MyColorjitter(0.07, 0.07, 0.07, 0.07),
        ...
        if dataset == 'cpm':
            data_set = HoverNetDataset_CPM(arg.DATASET_FP, arg.img_size, tranfs=[
                    MyRandomCrop((256,256)),
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.6)),
                    MyColorjitter(0.07, 0.07, 0.07, 0.07),
...
class Training_process:
    def __init__(self) -> None:
        ...
        self.lrs = MultiStepLR(self.optimizer, arg.lr_s, gamma=0.5)
    ...
    def run(self):
        for _ in range(arg.epoch):
            ...
            if _ % 30 == 6:
                ious, acces,losses = _val(self.test_data, self.net)
                writer.add_scalar("Hover:ious", ious, _)
                writer.add_scalar("Hover:loss", losses, _)
                writer.add_scalar("Hover:category acc", acces, _)
                # if best_iou < ious or best_acc < acces:
                print(
                    f"best_iou:{ious:.4f},best_acc:{acces:.4f},losses:{losses:.4f},\n epoch:{_} saving model "
                )
                torch.save(
