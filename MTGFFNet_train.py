...
import json
import torch
from Netlib.MTGFFlib.config import mtgff_config,setup_seed
from Netlib.CDNetlib.tools.augmentation import(
    Random_flip,
    MyGausssianBlur,
    MyColorjitter,
    MyRandomCrop,
...
from Netlib.MTGFFlib.tools.dataset import (
    DataLoader,
    Subset,
    MtgffMonusegDataset,
    MtgffCPMDataset,
...
from Netlib.MTGFFlib.tools.visualize_utils import Visualizer
...
parser = argparse.ArgumentParser(description="train MTGFFNet")
parser.add_argument("--ds", type=str, default="cpm")
parser.add_argument("--device", type=str, default="cuda")
parser.add_argument("--split_num", type=int, default=1)
parser.add_argument("--config", type=str, default="MTGFFCPM4")
process_arg = parser.parse_args()
arg = getattr(mtgff_config, process_arg.config)(
    ds=process_arg.ds,
...
def _train(
    ...
    losses = []
    for data in bar:
        imgs, labels, boxes, masks, cls, semantic_map = data
        ...
        semantic_map = semantic_map.to(device = process_arg.device)

        loss: dict = model(imgs, boxes, masks, cls, semantic_map)
        opt.zero_grad()
        ...
        bar.set_description(f"Epoch:{i},losses:{loss_.item():.4f},{loss},lr:{lr_s.get_last_lr()}")
...
def _val(
    ...
    for i, data in enumerate(bar):
        ...
        if len(boxes[0]) == 0:
            continue
        v = Visualizer(imgs[0], labels[0].long(), label=Visualizer.CONIC_CLASSES)
        ...
        with autocast():
            preds, metircs = model(imgs, boxes, masks, cls, semantic)
...
class Moudule_mode:
    def __init__(self, dataset) -> None:
        ...
        if dataset == "conic":
            self.train_set = MtgffConicDataset(
                dataset_dir=arg.DATASET_FP,
                masks_size=arg.ROI_RESOLUTION,
                img_size=arg.img_size,
                transfs=[
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.3)),
                    MyColorjitter(0.1, 0.1, 0.1, 0.1),
                ],
                train=True,
            )
            self.train_set = Subset(self.train_set, arg.TRAIN_SET_INDEX)
            self.val_set = MtgffConicDataset(
                dataset_dir=arg.DATASET_FP,
                masks_size=arg.ROI_RESOLUTION,
                img_size=arg.img_size,
                transfs=[
                    MyCenterCrop((256,256))
                ],
                train=True
            )
            self.val_set = Subset(self.val_set, arg.TEST_SET_INDEX)

        if dataset == "pannuke":
            self.train_set = MtgffPannukeDataset(
                dataset_dir=arg.DATASET_FP,
                masks_size=arg.ROI_RESOLUTION,
                img_size=arg.img_size,
                transfs=[
            ...
            self.val_set = Subset(self.train_set, arg.TEST_SET_INDEX)

        if dataset == "monuseg":
            self.train_set = MtgffMonusegDataset(masks_size=arg.ROI_RESOLUTION,img_size=arg.img_size,transfs=[
                    MyRandomCrop((256,256)),
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.3)),
                    MyColorjitter(0.1, 0.1, 0.1, 0.1),
            ],train=True)
            self.val_set = MtgffMonusegDataset(
                masks_size=arg.ROI_RESOLUTION,
                img_size=arg.img_size,
                transfs=[
                    MyCenterCrop((256,256))
            ...
            self.val_set = Subset(self.val_set, arg.VAL_SET_INDEX)
        if dataset == "cpm":
            self.train_set = MtgffCPMDataset(arg.DATASET_FP, masks_size=arg.ROI_RESOLUTION, img_size=arg.img_size, transfs=[
                    MyRandomCrop((256,256)),
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.3)),
                    MyColorjitter(0.1, 0.1, 0.1, 0.1),],train=True)
            self.val_set = MtgffCPMDataset(
                arg.DATASET_FP,
                masks_size=arg.ROI_RESOLUTION,
                img_size=arg.img_size,
                transfs=[
        ...
        if dataset == "dsb":
            self.train_set = MtgffDSBdataset(arg.DATASET_FP, masks_size=arg.ROI_RESOLUTION, img_size=arg.img_size, transfs=[
                    MyRandomCrop((256,256)),
                    Random_flip(),
                    MyGausssianBlur(3, sigma=(0.2, 0.3)),
                    MyColorjitter(0.1, 0.1, 0.1, 0.1),],train=True)
            
            self.val_set = MtgffDSBdataset(
                arg.DATASET_FP,
                masks_size=arg.ROI_RESOLUTION,
                img_size=arg.img_size,
                transfs=[
...
class Traning_process:
    def __init__(self) -> None:
        ...
        self.lrs = MultiStepLR(self.optimizer, arg.lr_s, gamma=arg.gamma)
    ...
    def generating_data(self):
        ...
        train_data = DataLoader(
            ...
            shuffle=True,
            num_workers=8,
            collate_fn=collect_fn_semantic,
        )
        val_data = DataLoader(
            val_set, 1, shuffle=False, num_workers=4, collate_fn=collect_fn_semantic
        )
