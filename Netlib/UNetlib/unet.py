...
import os
from Netlib.CDNetlib.loss import DiceLoss
...
class Unet(nn.Module):
    ...
    def forward(self, input, target=None):
        ...
        x_final_mask = self.mask_conv(x)
        
        if not target:
            losses = dict()
            losses["classes"] = 1.5*(focal_loss(
                x_classes, target["classes"]
            ) + self.dice_loss(x_classes, target["classes"]))
            if not self.training:
                x_classes = torch.argmax(x_classes, dim=1)  # n,h,w
                x_final_mask = torch.argmax(x_final_mask, dim=1)  # n,h,w
                x_direction = torch.argmax(x_direction, dim=1)  # n,h,w
                return [x_classes, x_final_mask, F.sigmoid(x_point).squeeze(1), x_direction],losses
            else:
                return losses
    ...
    def inference(self, img):
        ...
        for skip_name, upsample_block in zip(
            ...
            skip_features = features[skip_name]
            x = upsample_block(x, skip_features)
        x_C = self.classes_head(x)
        x_classes = self.final_conv(x_C)
