...
from infer import HovernetInfer
...
arg = getattr(hovernet_config, process_arg.config)(process_arg.ds,process_arg.device,process_arg.ds_num)
...
class InferEngine:
    def __init__(self) -> None:
        ...
        self.infer = HovernetInfer(arg=arg)
    ...
    def print_metric(self, pred_path, true_path):
        ...
        pred = torch.from_numpy(pred.astype(np.uint16)).long() # [N, 256, 256, 2]
        target = torch.from_numpy(target.astype(np.uint16)).long() # [N, 256, 256, 2]
