...
from Netlib.CenterMasklib.centermask.utils.comm import get_world_size
from fvcore.nn import sigmoid_focal_loss_jit

from Netlib.CenterMasklib.centermask.utils.comm import reduce_sum
from Netlib.CenterMasklib.centermask.layers import ml_nms
...
def fcos_losses(
    ...
    # prepare one_hot
    # print(labels[pos_inds].bincount())
...
class FCOSOutputs(object):
    ...
    def _get_ground_truth(self):
        ...
        loc_to_size_range = []
        for l, loc_per_level in enumerate(self.locations):  # (4096,2)
    ...
    def get_sample_region(self, gt, strides, num_loc_list, loc_xs, loc_ys, radius=1):
        ...
        center_x = (gt[..., 0] + gt[..., 2]) / 2  # 1364 ,n
        center_y = (gt[..., 1] + gt[..., 3]) / 2
        center_gt = gt.new_zeros(gt.shape)  # 1364,n,4
        # no gt
        ...
        for level, num_loc in enumerate(num_loc_list):
            ...
            xmin = center_x[beg:end] - stride  # 1364,n
            ...
            beg = end
        left = loc_xs[:, None] - center_gt[..., 0]  # 1364,1 - 1364,88
        right = center_gt[..., 2] - loc_xs[:, None]
        top = loc_ys[:, None] - center_gt[..., 1]
        bottom = center_gt[..., 3] - loc_ys[:, None]
        center_bbox = torch.stack((left, top, right, bottom), -1)  # 1364,n,4
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges):
        ...
        for im_i in range(len(targets)):
            ...
            reg_targets_per_im = torch.stack([l, t, r, b], dim=2)  # 對每個像素點，計算到每個gt的距離

            if self.center_sample:
                is_in_boxes = self.get_sample_region(
                    bboxes, self.strides, self.num_loc_list, xs, ys, radius=self.radius
                )
                # print('有多少個正例center',is_in_boxes.sum())
            else:
            ...
            locations_to_gt_area[is_in_boxes == 0] = INF  # 沒有gt的地方設為INF
            locations_to_gt_area[is_cared_in_the_level == 0] = INF  # 超出範圍的地方設為INF

            # if there are still more than one objects for a location,
            # we choose the one with minimal area
            # reg計算gt面積最小的框作爲正例，分類計算所有像素點
    ...
    def predict_proposals(self):
        ...
        for i, (l, o, r, c, s) in enumerate(zip(*bundle)):
            ...
            sampled_boxes.append(
                self.forward_for_single_feature_map(l, o, r, c, self.image_sizes)
            )
    ...
    def forward_for_single_feature_map(
        self, locations, box_cls, reg_pred, ctrness, image_sizes
    ):
