...
def instance2results(preds: List[dict], target: List[dict]):
    ...
    for pred_per_img, gt_per_img in zip(preds, target):
        pred_boxes = pred_per_img["instances"].pred_boxes.tensor.cpu()
        if len(pred_boxes) == 0:
            continue
        if not gt_per_img["instances"]:
            continue
        gt_boxes = gt_per_img["instances"].gt_boxes.tensor

        pred_masks = pred_per_img["instances"].pred_masks.cpu()
        gt_mask = gt_per_img["instances"].gt_masks.polygons

        gts = torch.zeros(len(gt_mask), 256, 256)
        for _, p in enumerate(gt_mask):
            p = torch.from_numpy(polygons_to_bitmask(p, 256, 256))
            gts[_] = p
        gt_cls = gt_per_img["instances"].gt_classes
        pred_cls = pred_per_img["instances"].pred_classes.cpu() + 1

        iou = box_iou(pred_boxes, gt_boxes)
        pr,re,f1 =compute_box_pre_rec_f1(pred_boxes,gt_boxes,[0.5,0.6,0.7]) 
        max_iou, index = torch.max(iou, dim=-1)
        prc.append(pr)
        recall.append(re)
        f1s.append(f1) #n,3
    ...
    f1s = torch.tensor([0.,0.,0.]) if len(f1s)==0 else torch.tensor(f1s).mean(0).nan_to_num(0)
    prc = torch.tensor([0.,0.,0.]) if len(prc)==0 else torch.tensor(prc).mean(0).nan_to_num(0)
    recall = torch.tensor([0.,0.,0.]) if len(recall)==0 else torch.tensor(recall).mean(0).nan_to_num(0)
    return (
