...
class MyRandomCrop(nn.Module):
    def __init__(self, size, padding=None, pad_if_needed=False, fill=0, padding_mode="constant"):
        super().__init__()
        self.rc = RandomCrop(size=size,padding=padding,pad_if_needed=pad_if_needed,fill=fill,padding_mode=padding_mode)

    def forward(self, *args):
        res = []
        for i in args:
            res.append(self.rc(i))
        return res

class MyCenterCrop(nn.Module):
    def __init__(self, size):
        super().__init__()
        self.cc = CenterCrop(size=size)
    
    def forward(self, *args):
        res = []
        for i in args:
            res.append(self.cc(i))
        return res
...
class Randomrotation_vh:
    def __init__(self) -> None:
        self.range = [90.0, 180.0, 270.0]

    def __call__(self, *args):
        res = []
        index = random.randint(0, 2)
        angel = self.range[index]
        for i in args:
            res.append(TF.rotate(i, angel))
        return res
    ...
    def __call__(self, *args):
        ...
        shift = (
            torch.randn(
                (
                    2,
                    int((size[-2] - 1) / self.grid_size) + 1,
                    int((size[-1] - 1) / self.grid_size) + 1,
                )
            )
            * self.sigma
        )
...
class MyColorjitter(nn.Module):
    ...
    def forward(self, *args):
        return [
            self.c(args[idx]) if idx == 0 else args[idx]
            for idx in range(len(args))
        ]


class MyGausssianBlur(nn.Module):
    def __init__(self, kernel_size=3, sigma=...):
        super().__init__()
        self.g = GaussianBlur(kernel_size, sigma)
        

    def forward(self, *args):
        return [
            self.g(args[idx]) if idx == 0 else args[idx]
            for idx in range(len(args))
        ]
