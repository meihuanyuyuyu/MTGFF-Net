...
class RPN_(nn.Module):
    def __init__(
        ...
        self.conv_cls = nn.Sequential(
            *[bn_act_conv(256, 256, 3, 1, 1) for _ in range(3)]
        )
        if use_deform:
            self.conv_offset = nn.Conv2d(256, 3*3*3, 3, 1, 1)
            self.dcn = DeformConv2d(256,256, 3, 1, 1)
            self.conv_reg = nn.Sequential(*[bn_act_conv(256, 256, 3, 1, 1) for _ in range(1)])
        else:
    ...
    def compute_rpn_loss(
        ...
        if use_giou:
            # 锚框无法使用giouloss计算梯度，故不使用
            raise("giou loss can't be used in rpn")
            loss_reg = giou_loss(
        ...
        # if torch.isnan(loss_logist) or torch.isnan(loss_reg):
        #     print("reg loss or logist loss is nan")
        # print(loss_logist,loss_reg)
        return {"rpn_loss": 5 * loss_reg + 1.25* loss_logist}
    ...
    def forward(self, x: OrderedDict, target_boxes: List[torch.Tensor] = None):
        ...
        for l in self.stride:
            ...
            if self.use_deform:
                offset, mask = torch.split(self.conv_offset(feature),(2*3*3,3*3),1)
                reg_tower = self.dcn(feature, offset, mask)
            else:
        ...
        if self.training:
            losses = self.compute_rpn_loss(
                logist_out, reg_out, target_boxes=target_boxes, use_giou= self.use_giou
            )
