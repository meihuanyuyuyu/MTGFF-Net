...
from .Backbone import Resnet50FPN, Resnet101FPN, FPNwithBottomup
from .MaskHead import MaskHead
from .BoxHead import BoxHead, BoxRefinement
from .GSS import GlobalSemanticSeg
from .GFF import FeatureFusion
...
class MTGFFNet(nn.Module):
    def __init__(
        ...
        if use_semantic:
            ...
            if not fuse_feature:
                self.graph_feature_fusion =  FeatureFusion(output_size=14)
            else:
                self.graph_feature_fusion = fuse_feature(output_size = 14)
        else:
    ...
    def compute_detection_loss(
        ...
        if len(cls) != 0:
            ...
            if (cls!=0).sum() == 0:
                ...
                loss = 3 * cls_loss + mask_loss + 8 * reg_loss
        else:
            loss = None
    ...
    def forward(
        ...
        if self.use_H_stain:
            h_img = np.zeros((img.shape[0],img.shape[-2],img.shape[-1],1))
            for idx in range(len(img)):
                h_img[idx] = rgb2hed(img[idx].permute(1,2,0).cpu().numpy())[...,0:1]
            img = torch.cat([img,torch.from_numpy(h_img).permute(0,3,1,2).to(device=img.device,dtype=img.dtype)],dim=1) # n,4,h,w
            img = self.conv1(img)
        # resnet+fpn:
        ...
        if self.training:
            ...
            # GSS train:
            if self.global_semantic is not None:
                semantic_features, pred_semantic_mask, loss = self.global_semantic(semantic_features, self.seg_stride, semantic)
                losses.update(loss)
                roi_feature = self.graph_feature_fusion(roi_feature, semantic_features, batched_rois)
            
            # final mask prediction(Mask Head):
        ...
        # inference:
            ...
            detection_cls = self.category_detection(roi_feature)
            if self.global_semantic is not None:
                semantic_feature, pred_semantic_mask, _ = self.global_semantic(semantic_features, self.seg_stride)
                fused_roi_feature = self.graph_feature_fusion(roi_feature, semantic_feature, batched_rois)
