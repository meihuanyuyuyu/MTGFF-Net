...
class BoxHead(nn.Module):
    def __init__(self, in_channels, out_channels=4, out_size=14):
        ...
        #     bn_act_conv(128, 32, 3, 1, 1 ),
        # )
        # self.block_reg2 = bn_act_conv(32, 32, 1, 1)
        self.block_reg = nn.Sequential(
            bn_act_conv(in_channels, 128, 3, 1, 1),
        ...
        self.fc = nn.Linear(
            out_size * out_size * 16,
            out_features=out_channels,
        )

    def forward(self, x):
        reg_x = self.block_reg(x)
        #reg_x = self.block_reg2(reg_x) + reg_x
        # 256-> 64*7**2
...
class BoxRefinement(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.box_head = box_head(256, out_channels=4, out_size=out_size)
