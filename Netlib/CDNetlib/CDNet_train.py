import argparse
import torch
from Config import CDNet_config, setup_seed, color
from tools.logger import Logger
from tools.augmentation import Random_flip, MyGausssianBlur, MyColorjitter
from CDNetlib import CDNetDataset, model_unet_MandDandP,DataLoader, Subset
...
parser.add_argument("--config", type=str, default="CDNet_C")
...
def _val(
    ...
    logger.info(f"val_metric:{metrics},epoch:{epoch}")
...
if __name__ == "__main__":
    ...
    net = model_unet_MandDandP.Unet(
        arg.BACKBONE_NAME, classes=arg.CLASSES, pretrained=False
    )
    ...
    for epoch in range(arg.EPOCHS):
        ...
        if epoch % arg.VAL_EPOCH == 1:
            ...
            if metric["classes"] > best_metric_c:
                best_metric_c = metric["classes"]
